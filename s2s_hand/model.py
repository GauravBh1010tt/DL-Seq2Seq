...
class model(nn.Module):
    def __init__(self, input_size, hidden_size, num_gaussian, dropout_p = 0.05, n_layers=1):
        ...
        self.gru2 = nn.GRU(hidden_size*bi_add+input_size, hidden_size, n_layers, dropout=dropout_p, batch_first=False, bidirectional = True)
...
for i in range(1,epochs+1):
  ...
  input_tensor = torch.tensor(data_x[i-1], dtype=torch.float, device=device)
  target_tensor = torch.tensor(data_y[i-1], dtype=torch.float, device=device)
