...
class model(nn.Module):
    ...
    def forward(self, inp, hidden1, hidden2):
        ...
        e_t = y_t[:,:,0:1]
        pi_t, mu1_t, mu2_t, s1_t, s2_t, rho_t = torch.split(y_t[:,:,1:], self.num_gaussian, dim=2)
...
def mdn_loss(mdn_params, data):
    ...
    #print (data)
    eos, x1, x2 = data.narrow(-1,0,1), data.narrow(-1,1,1), data.narrow(-1,2,1)
    ...
    res1 = torch.sum(torch.mul(pi_t,res),dim=2)
    #print(res1)
    ...
    res2 = -torch.log(res2).squeeze()
    #print (res2.shape)
    #res_final = v*torch.sum(res1+res2,dim=1)
    return torch.sum(res1+res2)

def sample(lr_model, num=1000, scale = 20, random_state=888):
    ...
    for i in range(num):
        ...
        idx = get_pi_idx(np.random.random(), mdn_params[1][0][0])
        eos = 1 if np.random.random() < mdn_params[0][0] else 0
        #print (idx)
    
        next_x1, next_x2 = sample_gaussian_2d(mdn_params[2][0][0][idx].detach().cpu().numpy(), 
                            mdn_params[3][0][0][idx].detach().cpu().numpy(), 
                            mdn_params[4][0][0][idx].detach().cpu().numpy(), 
                            mdn_params[5][0][0][idx].detach().cpu().numpy(), 
                            mdn_params[6][0][0][idx].detach().cpu().numpy())
...
for big_epoch in range(30):
    ...
    for i in range(0,epochs,batch_size):
      ...
      mdn_params, hidden1, hidden2 = lr_model( input_tensor, hidden1, hidden2)
      #bre
      md.append(mdn_loss(mdn_params, target_tensor))
      #print (md)
      if torch.isnan(md[-1]):
          bre
      #los += md[-1]
      loss = mdn_loss(mdn_params, target_tensor)/(input_tensor.size()[1])
      #print (loss/100)
